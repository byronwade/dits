...
impl OutgoingTransfer {
    ...
    pub fn transfer_id(&self) -> u64 { self.transfer_id }
    pub fn file_info(&self) -> &FileInfo { &self.file_info }
    ...
    pub fn is_complete(&self) -> bool { self.chunks_sent >= self.file_info.chunk_count }
}
...
impl IncomingTransfer {
    pub fn new(transfer_id: u64, file_path: std::path::PathBuf, file_size: u64, file_hash: ContentHash, chunk_count: u32) -> Self {
        Self {
            transfer_id, file_path, file_size, file_hash, chunk_count,
            chunks_received: vec![false; chunk_count as usize], bytes_received: 0,
        }
    }
    ...
    pub fn missing_chunks(&self) -> Vec<u32> {
        self.chunks_received.iter().enumerate().filter(|(_, &received)| !received).map(|(i, _)| i as u32).collect()
    }

    pub fn is_complete(&self) -> bool { self.chunks_received.iter().all(|&x| x) }
    ...
    pub fn verify_file(&self) -> std::io::Result<bool> {
        ...
        Ok(hasher.finalize() == self.file_hash)
    }
}
