...
Dits has no supported self-hosted server distribution. dits serve is unauthenticated, binds to loopback by default, and is only for trusted or isolated networks when rebound; it does not enable repository transfer commands.
