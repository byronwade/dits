...
## Task 3: Configure components.json for v4 + the @byronwade registry
...
Replace the file with (keeps New York style + aliases, drops the `tailwind.config` path which v4 doesn't use, registers the namespace):
...
- [ ] **Step 2: Verify the namespace resolves**
...
git commit -m "build: register @byronwade registry and v4 conventions in components.json"
```

---

## Task 4: Install foundation tokens and preserve app CSS

This is the most delicate task. `globals.css` currently has TWO parts:
- **Lines 1–107:** the old v3 HSL token layer (`@tailwind` directives + `:root`/`.dark`). This is REPLACED by the foundation.
- **Lines 108–708:** hand-rolled app CSS — `.container`, `.no-scrollbar`, `.hljs`/code styling, smooth-scroll, `.skip-link`, focus rings, `.tabular-nums`, `.overscroll-contain`, the `@keyframes ellipsis`, and ~250 lines of `.prose` typography that 80 docs pages depend on. This MUST be PRESERVED and converted to v4.

- [ ] **Step 1: Back up the current app CSS (lines 108–end) to a scratch file**
...
```
Expected: ~600 lines captured.

- [ ] **Step 2: Run the foundation init to write the v4 token layer**

```bash
cd /Users/byronwade/dits/apps/web
npx shadcn@latest init https://ui.byronwade.com/r/foundation.json
```
When prompted, accept overwriting `globals.css` (the token layer is what we want) and keep existing `components.json`. This writes `@import "tailwindcss";`, `@import "tw-animate-css";`, the `:root`/`.dark` OKLCH tokens, the `@theme inline` mappings, the radius scale, and house utilities (`bg-grid`, `glow-brand`, `text-gradient`, `shadow-card`, etc.).

- [ ] **Step 3: Re-append the preserved app CSS, converting v3 remnants**
...
git commit -m "style: install byronwade/ui foundation tokens, preserve app + prose CSS (v4)"
```
...
These have ZERO usage outside `src/components/ui/` (verified by grep). Delete them:
...
rm -f alert-dialog.tsx button-group.tsx calendar.tsx carousel.tsx context-menu.tsx \
  drawer.tsx empty.tsx field.tsx form.tsx input.tsx input-otp.tsx item.tsx kbd.tsx \
  label.tsx menubar.tsx pagination.tsx popover.tsx progress.tsx radio-group.tsx \
  resizable.tsx scroll-area.tsx select.tsx separator.tsx sidebar.tsx skeleton.tsx \
  slider.tsx spinner.tsx switch.tsx textarea.tsx toggle.tsx toggle-group.tsx \
  aspect-ratio.tsx avatar.tsx checkbox.tsx collapsible.tsx command.tsx \
  hover-card.tsx navigation-menu.tsx
```
NOTE: do NOT delete `code-block.tsx`, `chart.tsx` (used by benchmarks), or any of the 13 used components. After deleting, if `@byronwade/all` re-added some of these as composites' dependencies and they ARE imported by a composite, the build in Step 4 will flag the missing import — restore only those.
