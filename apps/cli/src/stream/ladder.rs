//! ABR rendition ladder: a set of (resolution, bitrate) rungs the player can
//! adapt between. Every rung encodes the same canonical frames at a different
//! scale/bitrate; the frame-diff (and thus the incremental reuse plan) is
...
/// The default ladder, filtered to rungs at or below `source_height` (never
/// upscales). If every default rung is taller than the source, fall back to a
/// single source-height rung.
pub fn default_ladder(source_height: u32) -> Vec<Rendition> {
    let full = [("720p", 720u32, 2500u32), ("480p", 480, 1200), ("240p", 240, 500)];
    let mut rungs: Vec<Rendition> = full
        .iter()
        .filter(|(_, h, _)| *h <= source_height)
        .map(|(name, h, b)| Rendition {
            name:         (*name).to_string(),
            height:       *h,
            bitrate_kbps: *b,
        })
