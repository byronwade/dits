//! Tiny axum origin: serves a master playlist + per-rendition media playlists,
//! content-addressed CMAF segments, a stats JSON, and the hls.js player page.
...
use crate::stream::{
    ...
    playlist::{master_playlist, StreamVersion},
};
...
/// Master playlist for a version: one STREAM-INF per rung ->
/// /r/{version}/{rung}.m3u8.
async fn master(
...
async fn segment(
    ...
    // Content-addressed: hash is the part before the extension (.mp4 init or .m4s
    // media).
...
async fn stats(State(s): State<Arc<ServeState>>) -> impl IntoResponse {
    // Aggregate reuse across every rung: a v2 rung segment is reused if its hash
    // appears in the matching v1 rung.
    ...
    for (r2, sv2) in &s.v2 {
        ...
        reused += sv2
            .segments
            .iter()
            .filter(|x| v1_hashes.contains(&x.hash))
            .count();
    }
