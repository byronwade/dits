//! Per-shot VMAF-targeted encoding: choose each segment's CRF to hit a perceptual-quality target
//! (Netflix Dynamic Optimizer in miniature). Every step is deterministic — same frames produce the
//! same chosen CRF and byte-identical output — so content-addressing and incremental reuse hold.
...
/// CRF search bounds. Lower CRF = higher quality/bigger; higher CRF = lower quality/smaller.
pub const MIN_CRF: u32 = 18;
pub const MAX_CRF: u32 = 34;

/// Write frame blobs to a temp dir as `f_%08d.<ext>`; returns the dir (caller removes it).
...
/// Encode staged frames to a plain mp4 at `crf` (same libx264 settings as the segment encoder, so
/// measured quality matches the delivered bitstream).
fn encode_probe(dir: &Path, frame_rate: &str, frame_ext: &str, crf: u32, out: &Path) -> Result<()> {
    ...
    let status = Command::new("ffmpeg")
        ...
        .args([
            ...
            "-sc_threshold", "0", "-an", "-crf", &crf.to_string(),
        ])
        .arg(out)
        .output()
...
/// Build a near-lossless reference video from staged frames (the VMAF reference).
...
/// Highest CRF (smallest size) whose encode scores VMAF >= `target`. Returns `(crf, achieved_vmaf)`.
/// Deterministic bounded binary search over [MIN_CRF, MAX_CRF]. If even MIN_CRF misses the target,
/// returns MIN_CRF with its (best-effort) score.
pub fn optimize_crf(
    frames: &[Vec<u8>],
    frame_rate: &str,
    frame_ext: &str,
...
mod tests {
    ...
    /// `source` is a complete lavfi spec, e.g. "mandelbrot=size=320x240:rate=10".
    ...
    fn simple_content_uses_higher_crf_than_complex() {
        if !ffmpeg_available() { eprintln!("skipping: no ffmpeg"); return; }
        // A flat color clip is trivially compressible -> the optimizer can use a higher CRF (smaller)
        // than detailed content at the same VMAF target.
        ...
        assert!(crf_simple >= crf_complex, "simple crf {crf_simple} should be >= complex crf {crf_complex}");
    }
}
