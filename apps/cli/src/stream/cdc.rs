//! Content-defined segment boundaries over the frame-hash sequence (FastCDC-style). A boundary is
//! chosen by content, not by a fixed frame count, so a trim/insert that shifts frame positions only
//! disturbs the boundary near the edit — segments before and after re-synchronise to the same
//! boundaries and keep the same content id, preserving incremental reuse across edits.
...
pub struct CdcSegment {
    ...
    /// `blake3(concat of the segment's frame hashes)` — equal iff the frame run is identical.
    pub content_id: Hash,
}

/// Is this frame a content boundary? Frame hashes are already uniformly random (BLAKE3), so the
/// predicate is a pure function of the frame's own hash — no rolling hash needed, and every
/// unchanged frame keeps its boundary status regardless of position, which is what gives perfect
/// resync after a trim/insert. Boundary probability is `1/avg` (via `mask`).
...
pub fn cdc_segments(manifest: &ClipManifest, p: &CdcParams) -> Vec<CdcSegment> {
    ...
    for i in 0..n {
        count += 1;
        let cut = (count >= p.min && is_boundary_frame(&manifest.frames[i].hash, mask)) || count >= p.max;
        if cut || i == n - 1 {
            let end = i + 1;
...
mod tests {
    ...
    fn insert_only_disturbs_the_boundary_region() {
        ...
        // CDC re-synchronises: most segments survive the insert (vs ~0 for fixed boundaries, which
        // would shift every segment after index 30).
        assert!(
        ...
        let first_changed = s2.iter().position(|s| !ids1.contains(&s.content_id)).unwrap();
    ...
    fn fixed_boundaries_would_lose_reuse_after_an_insert() {
        // Contrast: with FIXED boundaries (chunk of 8), an insert at 30 shifts every later chunk.
        let l = labels(60);
        ...
        assert!(shared <= 4, "fixed boundaries should lose most reuse after an insert, kept {shared}");
    }
}
