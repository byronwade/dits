...
//!
//! This command allows managing multiple working trees attached to the same
//! repository.
...
fn load_worktree(wt_admin_dir: &Path) -> Result<Option<Worktree>> {
    ...
    let worktree_path = fs::read_to_string(&gitdir_file)?.trim().to_string();
...
pub fn add(path: &str, branch: Option<&str>, new_branch: bool, force: bool) -> Result<Worktree> {
    ...
    if worktree_path.exists() && !force {
        anyhow::bail!("Path '{}' already exists. Use --force to overwrite.", path);
    }
    ...
    fs::write(wt_admin_dir.join("gitdir"), wt_dits_dir.to_string_lossy().as_ref())?;

    // Create .dits file in worktree (points back to main repo)
    fs::write(&wt_dits_dir, format!("gitdir: {}", wt_admin_dir.display()))?;
...
pub fn remove(path: &str, force: bool) -> Result<()> {
    ...
    let admin_dir =
        found_admin_dir.ok_or_else(|| anyhow::anyhow!("Worktree '{}' not found", path))?;

    // Check if locked
    let locked_file = admin_dir.join("locked");
    if locked_file.exists() && !force {
        let reason = fs::read_to_string(&locked_file).unwrap_or_default();
        anyhow::bail!(
...
pub fn print_list(worktrees: &[Worktree], verbose: bool) {
    for wt in worktrees {
        ...
        if verbose {
            ...
            println!("{} {}{}", wt.path.display(), wt.head, status);
        }
    }
}
