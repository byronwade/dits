...
use crate::{
    hooks::{get_hooks_dir, get_sample_hook, HookType},
    store::Repository,
};
...
pub fn install(hook_name: &str, force: bool) -> Result<()> {
    ...
    let hook_type = HookType::from_str(hook_name).ok_or_else(|| {
        anyhow::anyhow!(
            "Unknown hook: {}. Use 'dits hooks list' to see available hooks.",
            hook_name
        )
    })?;
    ...
    if hook_path.exists() && !force {
        anyhow::bail!("Hook '{}' already exists. Use --force to overwrite.", hook_name);
    }
