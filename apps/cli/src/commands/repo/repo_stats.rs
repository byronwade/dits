...
pub fn repo_stats(verbose: bool) -> Result<()> {
    ...
    println!("  Message: {}", style(&commit.message).dim());
    println!();

    // File summary
    println!("{}", style("Files:").bold());
    println!("  Tracked files: {}", dedup_stats.file_count);
    println!();
    ...
    println!(
        "  Logical size:  {} {}",
        style(format_bytes_with_raw(dedup_stats.logical_size))
            .white()
            .bold(),
    ...
    println!(
        "  Physical size: {} {}",
        style(format_bytes_with_raw(dedup_stats.physical_size))
            .white()
            .bold(),
    ...
    println!("  Unique chunks: {}", dedup_stats.unique_chunk_count);
    println!(
        "  Space saved:   {} ({:.1}%)",
        style(format_bytes_with_raw(dedup_stats.saved_bytes))
            .green()
            .bold(),
    ...
    if dedup_stats.savings_percentage > 50.0 {
        ...
        println!("  {} Good deduplication. Significant chunk reuse detected.", style("✓").green());
    } else if dedup_stats.savings_percentage > 5.0 {
        println!("  {} Moderate deduplication. Some chunk reuse detected.", style("○").yellow());
    } else {
        println!(
    ...
    if verbose {
        ...
        println!("  Total chunks:    {}", basic_stats.chunk_count);
        println!("  Total manifests: {}", basic_stats.manifest_count);
        println!("  Total commits:   {}", basic_stats.commit_count);
        println!("  Storage bytes:   {}", format_bytes_with_raw(basic_stats.storage_bytes));

        // Per-file breakdown
        println!();
        println!("{}", style("Per-File Breakdown:").bold());
        println!("  {:<40} {:>12} {:>8} {:>8}", "Path", "Size", "Chunks", "Type");
        println!("  {}", "-".repeat(72));
