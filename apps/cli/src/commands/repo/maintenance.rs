...
pub fn run(task: Option<&str>) -> Result<()> {
    ...
    let tasks_to_run: Vec<MaintenanceTask> = match task {
        Some(t) => {
            let task = MaintenanceTask::from_str(t).ok_or_else(|| {
                anyhow::anyhow!(
                    "Unknown task: {}. Use 'dits maintenance run' to see available tasks.",
                    t
                )
            })?;
            vec![task]
        ...
        None => {
            ...
            vec![MaintenanceTask::Gc, MaintenanceTask::CommitGraph]
        },
    };
