...
impl RendezvousClient {
    ...
    /// Register as a host with a join code and certificate fingerprint
    pub async fn host_with_fingerprint(
        &self,
        join_code: &str,
        ...
        let (mut ws, _) = timeout(self.timeout, connect_async(&self.server_url))
            .await
        ...
        let response = timeout(self.timeout, ws.next())
            .await
            .map_err(|_| RendezvousError::Timeout)?
        ...
        loop {
            let response = timeout(Duration::from_secs(60), ws.next())
                .await
                .map_err(|_| RendezvousError::Timeout)?
                .ok_or(RendezvousError::ConnectionFailed("Connection closed".into()))?
                .map_err(|e| RendezvousError::ConnectionFailed(e.to_string()))?;
    ...
    /// Connect to a host via join code
