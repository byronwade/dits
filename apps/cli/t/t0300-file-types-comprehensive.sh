...
write_magic_and_payload() {
	...
	# Write magic bytes then append a deterministic payload.
...
test_expect_success 'Archive system handles TAR files' '
	(
		cd filetypes-test &&
		# USTAR magic at offset 257, but we only need a recognizable header for this test.
...
test_expect_success '3D system handles Blender files' '
	(
		cd filetypes-test &&
		# Blender .blend starts with "BLENDER"
...
test_expect_success 'Game asset system handles Unreal Engine assets' '
	(
		cd filetypes-test &&
		# Unreal package files often start with 0xC1832A9E (little endian 9E2A83C1)
...
test_expect_success 'Scientific data system handles FITS files' '
	(
		cd filetypes-test &&
		# FITS headers are 80-char cards; start with "SIMPLE  =".
