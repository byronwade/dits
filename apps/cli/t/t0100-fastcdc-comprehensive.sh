...
if test "${GIT_TEST_LONG:-0}" != "1"
then
	test_skip "Long tests disabled (set GIT_TEST_LONG=1 to run t0100)"
	test_done
fi
...
test_expect_success 'FastCDC respects minimum chunk size' '
	(
		...
		# TODO: Verify chunk sizes explicitly.
		true
	)
'
...
test_expect_success 'FastCDC respects maximum chunk size' '
	(
		...
		# TODO: Verify max chunk sizes explicitly.
		true
	)
'
